...
/*   Updated: 2024/03/13 00:50:09 by zkotbi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void open_files(t_proc	*proc)
{
	...
	while (i < proc->nb_file)
	{
		if (proc->file[i].mod == INPUT)
			...
			proc->file[i].fd = open(proc->file[i].file_name, O_CREAT | O_WRONLY | O_TRUNC, 0777);
		else if (proc->file[i].mod == APPEND)
			proc->file[i].fd = open(proc->file[i].file_name, O_CREAT | O_WRONLY | O_APPEND, 0777);
		i++;
	}
}
